	...
	async function runTimeLineForMachineOrMachineGroup(data: TimeLineRunner) {
		...
		const timeLineHandlers =
			timeLines.find((timeLine) => timeLine.id === data.timeLineId)
                ...
                // Transform camelCase keys to PascalCase for .NET API
                const transformedHandlers = timeLineHandlers.map((h: any) => ({
                        HandlerType: h.handlerType,
                        Initial: h.initial,
                        UtcTimeOn: h.utcTimeOn,
                        UtcTimeOff: h.utcTimeOff === "24:00:00" ? "23:59:59" : h.utcTimeOff,
                        HandlerArgs: h.handlerArgs,
                        Loop: h.loop,
                        TimeLineEvents: (h.timeLineEvents ?? []).map((e: any) => ({
                                Command: e.command,
                                CommandArgs: e.commandArgs,
                                DelayAfter: e.delayAfter,
		...
		if (machine) {
                        await call(
                                fetch(`${process.env.NEXT_PUBLIC_GHOSTS_API_URL ?? window.location.origin.replace(":8080", ":5000")}/api/timelines`, {
                                        method: "POST",
                                        headers: { "Content-Type": "application/json" },
                                        body: JSON.stringify({
                                                machineId: machine.id,
                                                type: data.updateType,
                                                update: { Status: "Run", TimeLineHandlers: transformedHandlers },
                                        }),
                                }),
