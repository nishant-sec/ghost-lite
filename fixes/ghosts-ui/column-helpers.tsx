...
export function MachineGroupAddMachines({
	...
	const machinesNotInMachineGroup = allMachines.filter(
		(machine) =>
			!machineGroup.groupMachines?.find((machine2) => machine2.machineId === machine.id),
	);
	const currentGroupMachines = (machineGroup.groupMachines ?? []).map((machine) => ({
		groupId: machineGroup.id,
						...
						await api.putApimachinegroupsId(
							{
								...
								groupMachines: [
									...
									{ groupId: machineGroup.id, machineId: machine.id?.toString() },
								],
							},
							{
								params: { id: machineGroup.id?.toString() ?? "" },
